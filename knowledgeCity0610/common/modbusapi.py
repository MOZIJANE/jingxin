...
class tcpMaster(baseMaster):
	def __init__(self, slaveId, host, port=502, timeout=100.0):
...
class tcpServer:
	...
	def conn(self):
		...
		# 创建寄存器，寄存器起始地址为0，寄存器个数为16
		self.slave_1.add_block('block1', mdef.HOLDING_REGISTERS, 0, 16)

		self.slave_1.add_block('block2', mdef.HOLDING_REGISTERS, 16, 16)
