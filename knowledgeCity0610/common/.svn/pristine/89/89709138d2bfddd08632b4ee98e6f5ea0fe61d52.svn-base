...
class client:
	def __init__(self,name, serverIP, port,timeout,bindIp='',bindPort=0):
	...
	def doConnect(self,name):
		try:
			...
			self._socket.bind((self.bindIp,self.bindPort))
...
def test_robotclient():
	...
	for i in range(10):
		send("test",ip, port, "0" * 4000,timeout=15)
		data = recv("test",ip, port,4000)
		assert b'0' * 4000 == data
	tick = utility.ticks()
	try:
		log.info("start test")
		data = recv("test",ip, port,10,timeout=15)
		print(data)
