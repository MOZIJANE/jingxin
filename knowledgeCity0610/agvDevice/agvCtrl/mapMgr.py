...
class mapInfo:
	...
	def getPath(self,posStart, posEnd, agvId=None,writeLog=True):	
		posStart = formatLoc(self.mapId,posStart)
		posEnd = formatLoc(self.mapId,posEnd)
		if isSameMap(posStart,posEnd):
			if posStart == posEnd:
				return [(self.mapId,[],0),]
			w,pp,w2,pp2 = self.getPathSingle(posStart,posEnd,agvId)
			if len(pp) == 0:
				return []
			if pp2:
		...
		path = []
		for e in ee:   
			w1,path1,w11,path11 = getMap(map1).getPathSingle(pos1,e.findFloor(map1).loc,agvId) 
			w2,path2,w22,path22 = getMap(map2).getPathSingle(e.findFloor(map2).loc,pos2,agvId) 
			if w1 + w11 + w2 + w22 < w:
				w = w1 + w11 + w22 + w2
				path = [(map1,path1,w1),]
				if path11:
	...
	#返回：(权重1,路径1,权重2,路径2)
	def getPathSingle(self,posStart, posEnd, agvId):
		lines,oplines = [],[]
		if agvId:
			#getAgvPath这会返回的line字符串 
			lines,oplines = self.getAgvPath(agvId) 
	...
	def _loadOneway(self,filename,ctime): 
		...
		for c in self.curves: 
			t = utility.ticks()
			pp,ww = self._getPaths(c.endPosName,c.startPosName,stopCount=2,maxWeight=10+c.length)
			if len(pp) <= 1:
