...
class agvInfo:
	...
	def move(self, loc, timeout, taskId):
		assert loc
	...
	def moveJackup(self, loc, timeout, taskId):
		assert loc
		...
		self.status["target_id"] = loc
		agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="JackLoad",recognize=False,use_pgv=True)
		
		
	def moveJackdown(self, loc, timeout, taskId):
		assert loc
	...
	def _readStatus1(self):
		try:
			status = agvCtrl.readStatus1(self.agvId)  
			if status:
	...
	def _checkLocation(self,status):
		...
		if pos is not None and pos.name != self._curTarget: 
			...
			self._curTarget = pos.name 
			self.status = status
			targetLocEvent.emit(self.agvId, self._curTarget)
		self.status = status
		locationEvent.emit(self.agvId, self.x, self.y, self.status["angle"])
...
if __name__ == '__main__': 	
	clearTypeIds = getAlarmId()
	alarm.alarmApi.clearRange("AGV11", clearTypeIds, "agv",force=True)
	assert 0
