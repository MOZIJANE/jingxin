...
def goTargetAndDoWork(agvId, loc, operation, recognize=False,use_pgv=False, pgv_adjust_dist=False,ignoreAngle=None,pgv_type=1):
	...
	log.info(int(pgv_type))
	if int(pgv_type) == 3:
		dist = _load()['pgv_adjust_dist']
		data = {"id": loc, 'operation': operation, 'recognize': recognize, 'use_pgv': use_pgv, 'pgv_adjust_dist': dist}
	elif int(pgv_type) == 4:
		data = {"id": loc, 'operation': operation, 'recognize': recognize, 'use_pgv': use_pgv, 'pgv_adjust_dist': 0.3,
		        'pgv_adjust_cx': -0.1}
	elif int(pgv_type) == 1:
		data = {"id": loc, 'operation': operation, 'recognize': recognize, 'use_pgv': use_pgv}
...
def isForkType(agvId):
	if _getAgvType(agvId)['jackType'].lower() == "fork":
		return True
	else:
		return False
...
# forkup
def _forkUpOther(agvId, loc):

	agvApi.other(agvId, "robot_other_set_fork_height_req", {"height":1})

# forkdown
def _forkDownOther(agvId, loc):
	agvApi.other(agvId, "robot_other_set_fork_height_req", {"height": 0})
...
def jackUp(agvId,speed=5000,distance=60000,loc=None):
	if checkCfgIO(agvId):
		...
		jackUp2(agvId)
	elif isForkType(agvId):
		_forkUpOther(agvId,loc)
	else:
		# 常规方式
...
def jackDownStatus(agvId):
	if checkCfgIO(agvId):
		...
		return forkDownStatus(agvId)
	else:
		# 常规方式,工控机
		return agvApi.perpherial(agvId, "robot_other_robot11", None)

def forkUpOpen(agvId):
	s1=None
	status = readIOs(agvId)
	for x in status['DO']:
		if int(x['id']) == 2:
			s1 = x['status']
	if s1 is not None and s1 == False:
		data1 = {"id":1, "status":True}
		setDO(agvId, data1)
	else:
		raise Exception('cannot open s1 up')

def forkUpClose(agvId):
	data1 = {"id":1, "status":False}
	setDO(agvId, data1)


def forkDownOpen(agvId):
	s1=None
	status = readIOs(agvId)
	for x in status['DO']:
		if int(x['id']) == 1:
			s1 = x['status']
	if s1 is not None and s1 == False:
		data1 = {"id":2, "status":True}
		setDO(agvId, data1)
	else:
		raise Exception('cannot open s1 up')

def forkDownClose(agvId):
	data1 = {"id":2, "status":False}
	setDO(agvId, data1)

#def forkUpStatus(agvId):
#	ret= _forkUpStatus(agvId)
#	return {'status':ret}

#def forkDownStatus(agvId):
#	ret= _forkDownStatus(agvId)
#	return {'status':ret}

def _forkUpStatus(agvId):
	s1=None
	status = readIOs(agvId)
	for x in status['DO']:
		if int(x['id']) == 1:
	...
	for x in status['DI']:
		if int(x['id']) == 0:
			...
			r2 = x['status']
	if r1 is None or r2 is None or s1 is None:
		return 0
	else:
...
def setDO(agvId, data):
	return  agvApi.other(agvId,"robot_other_setdo_req",data)
