...
def _loadAgvList():
	...
	with open(file, 'r') as f:
		...
		for agvId in agvList:
			...
			agv.enable_block = agvType.getBool(agv.type,"enable_block",True)
			ret[agvId] = agv
...
class agvInfo:
	...
	def unlock(self,taskId,force=True):
		...
		self.totalTicks += (self.unlockTicks - self.lockTicks)/1000.
		self.lockTicks = 0
	...
	@property
	def oee(self):
		ti = self.totalTicks + self.lockSeconds
		if ti == 0:
			return 0
		t = datetime.datetime.now()
		total = float("%s.%s"%(t.hour*60*60 + t.minute*60 + t.second,t.microsecond))
		oee = ti/total if total != 0 else 0
		return oee
	...
	def moveJackup(self, loc, timeout, taskId, use_pgv=False,ignoreAngle=False):
		assert loc
		...
		pgv_adjust_dist = False
		self.requireControl()
		# use_pgv = False
		if self.isSeerRobot():

			if loc in ['AP22', 'AP25', 'AP18']:
				agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="JackLoad",recognize=False,use_pgv=use_pgv,pgv_adjust_dist=pgv_adjust_dist)
			elif loc in ['AP142', 'AP143', 'AP101']:
				agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="ForkLoad",recognize=False,use_pgv=use_pgv,pgv_adjust_dist=pgv_adjust_dist)
		else:

			agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="JackLoad",recognize=False,use_pgv=use_pgv,pgv_adjust_dist=pgv_adjust_dist,ignoreAngle=ignoreAngle)
	...
	def moveJackdown(self, loc, timeout, taskId,ignoreAngle=False):
		...
		if self.isSeerRobot():
			if loc in ['AP22', 'AP25', 'AP18']:
				...
				agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="ForkUnload",recognize=False,use_pgv=False)
	...
	def _checkAlarm(self):
		...
		def _sendAlarm2(alarms,clearTypeIds):
			for key in alarms:
				a = alarms[key]
				id = int(a["id"])
				alarm.alarmApi.alarm(moid=self.agvId, typeId=id, desc="AGV异常"+str(a), domain="agv")
				if id in clearTypeIds:
					clearTypeIds.remove(id)
				else:
					self._allAlarmId.add(id)
		...
		if self.isSeerRobot():
			...
			if "alarms" in self.status:
				_sendAlarm2(self.status["alarms"],clearTypeIds)
