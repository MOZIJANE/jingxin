...
class akmTask:
	...
	def filterFuncC(self,agvId, mapId, loc, payload):
		if agvId != local.get("line", self.line).split(',')[2]:
			return False
		s = self.getUnitUnloadStatus(agvId,self.floor)
		if s == 0:
			return True
	...
	def _callOtherAgv(self):
		agvId = local.get("line", self.line).split(',')[1]
		agvIdOther = local.get("line", self.line).split(',')[2]
		agvStatus = agvApi.getAgvStatus2(agvId,self.map)
		islocked = True
		isactive = True
		emergency = True
		islinked = True
		batterylevel = 0
		if "charging" in agvStatus :
			ischarge = agvStatus["charging"]
		if "isLocked" in agvStatus :
			islocked = agvStatus["isLocked"]
		if "isActive" in agvStatus :
			isactive = agvStatus["isActive"]
		if "isLinked" in agvStatus :
			islinked = agvStatus["isLinked"]
		if "emergency" in agvStatus :
			emergency = agvStatus["emergency"]
		if "battery_level" in agvStatus :
			batterylevel = agvStatus["battery_level"]
		# log.info("charging",ischarge,"isLocked",agvStatus["isLocked"],"emergency",agvStatus["emergency"],"battery_level",batterylevel )
		if agvIdOther != "AGV00":
			if ischarge and not emergency and not islocked and self.callagvCount < 15:
				try:
					self.callagvCount += 1
					return agvApi.apply(taskId=self.taskId, mapId=self.map, loc=self.source, filterFunc=self.filterFuncC,
										timeout=10)
				except Exception as e:
					log.warning("[%s] apply agv failed: %s" % (str(self), str(e)))
					return None
