...
class messageBox(QDialog):
	...
	def __init__(self,title,message,iconName,iconColor,buttons=["确定","取消"],fontSize=15):
		...
		l2 = QHBoxLayout()
		l2.setSpacing(20)
		def callBack(mboxCb,button):
			if mboxCb:
				mboxCb()
			self.onFinished.emit(button)
		for button in buttons:
			if button=='确定':
				cb=enhance.bind(callBack,self.accept,button)
				icon=qta.icon("fa.check", scale_factor=1.5, color="green")
			elif button=='取消':
				icon=qta.icon("fa.remove", scale_factor=1.5, color="red")
				cb=enhance.bind(callBack,self.reject,button)
			else:
				icon=None
				cb=enhance.bind(callBack,self.close,button)

			if icon:
				b = QPushButton(icon,button)
			else:
...
class progressBar:
	...
	def show(self):
		self.b = progressBox(self)
		if self.cancelFunc:
			self.b.setWindowFlags(Qt.WindowCloseButtonHint|Qt.MSWindowsFixedSizeDialogHint)
		else:
			self.b.setWindowFlags(Qt.WindowTitleHint|Qt.MSWindowsFixedSizeDialogHint)
		if self.b.exec() == self.b.Accepted:
			return True
		if self.cancelFunc:
...
def testmymbox():
	message = '自定义信息自定义信息自定义信息自定义信息自定义信息'
	def onFinished(b):
		info(b)
	b = messageBox("信息", message=message, iconName="fa.info-circle", iconColor="blue",
	               buttons=["确定", '自定义按钮1', '自定义按钮2', '自定义按钮3', '自定义按钮4'])
	b.onFinished.connect(onFinished)
	b.exec()
