...
class scadaTask:
	...
	def onFinish(self,obj):
		...
		if self.taskStep == "moveSource" or self.taskStep == "startMovePreSrc": 
			...
			taskMgr.update(self.taskId,failMsg="AGV前往目标点送货")
			return
			...
			taskMgr.update(self.taskId,failMsg="放下货架或等待取料")
			return 
...
#			lockImp.release(g_lock)
	...
	def _moveAgv(self,stepCallback=None):
		try:
			...
			agvApi.move(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, stepCallback=None)
			return True
		except (ConnectionResetError,ConnectionAbortedError,socket.timeout,IOError) as e:
			raise
		except Exception as e:
	...
	def _moveJackup(self):
		...
		if proName =='fastprint':
			if 'WorkPlace' in self.param and self.param['WorkPlace'] and self.param['WorkPlace']!='' and self.param['WorkPlace'].lower() == 'true':
				...
				agvApi.moveJackup(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, use_pgv=False,pgv_type=1)
		else:
			agvApi.moveJackup(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, self.param,use_pgv=False,pgv_type=1)
