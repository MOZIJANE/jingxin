...
class feed:
    def __init__(self, param):
        ...
        self._load(param)
        self.param["taskName"] = "从%s 运送货架到 %s"%(self.param["source"],self.param["target"])
    ...
    def _load(self,param):
        ds = db.find("u_loc_info",{"_id": {"$in": [param["source"],param["target"]]}})
        while ds.next():
            if ds["_id"] == param["source"]:
                self.s_info = {
    ...
    def _taskFunc(self):
        while self.isRunning:
            try:
                ...
                taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg=e)
                break
        time.sleep(1)
        self._release()
        self._goHome()
    ...
    def handle(self):
        if self.taskStep == "waiting":
            ...
            taskApi.update(self.taskId, status=taskApi.taskStatus.waiting.value, msg="等待分配小车")
            return True
            ...
            self.updateDB()
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="检测下降")
            return True
        elif self.taskStep == "initJackdown":
            # if not self._jackDown():     ##  seer don't need jackdown
            #     return False
            self.taskStep = "waitSrcPreLoc"
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="等待%s前置点锁"%self.s_info["prepoint"])
            return True
            ...
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="前往工位")
            return True
        elif self.taskStep == "goSource":
            if not self._moveJackup():
                return False
            ...
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="等待目标位置%s前置点锁"%self.t_info["prepoint"])
            return True
            ...
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="前往目标前置点")
            return True 
        elif self.taskStep == "goTarget":
            if not self._moveJackdown():
                return False
    ...
    def updateDB(self):
        db.update_one("u_loc_info",{"_id": self.param["source"]},{"status":0})
        db.update_one("u_loc_info",{"_id": self.param["target"]},{"status":2})
    ...
    def onFinish(self,obj):
        ...
        if not obj["result"]:
            ...
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg=obj["resultDesc"])
            return
        if self.taskStep == "goSourceMoving": 
            self.taskStep = "jackUpWP"
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="转盘顶升")
            return  
        elif self.taskStep == "goTargetMoving":
            self.taskStep = "jackDown"
            taskApi.update(self.taskId, status=taskApi.taskStatus.working.value, msg="转盘下降")
            return
    ...
    def _jackUp(self,timeOut=1800):
        try:    
            ...
            jackApi.jackUp(self.agvId, timeOut,agvInfo.curTarget)
            return True
        except (ConnectionResetError,ConnectionAbortedError,socket.timeout,IOError) as e:
            # self._deal(str(e))
            raise e
    ...
    def _jackDown(self, timeOut=1800):
        try:    
            agvInfo = agvList.getAgv(self.agvId) 
            jackApi.jackDown(self.agvId, timeOut,agvInfo.curTarget)
            return True
        except (ConnectionResetError,ConnectionAbortedError,socket.timeout,IOError) as e:
            # self._deal(str(e))
            raise e
