...
@scadaUtility.get('/api/web/activeAlarm',cacheSecond=5)
def urlAlarm():
	...
	for a in alarmList:
		...
		t.append(a["time"])
		t.append(a["desc"])
		t.append(getLevel(a["level"]))
		data.append(t)
	
	config = {}
	config["header"] = ["对象", "时间", "描述", "级别"]
	config["data"] = data
	config["rowNum"] = 10
	config["headerHeight"] = 35
	config["headerBGC"] = "#0f1325"
	config["oddRowBGC"] = "#0f1325"
	config["evenRowBGC"] = "#171c33"
	config["index"] = False
	config["columnWidth"] = [85,163,170]
	config["waitTime"] = 3000
	config["align"] = ['center','center','center','center']
	return {"title": "告警列表","config": config}

def getLevel(value):
	if value == "critical":
		return '<span style="color:#ff0000;">严重</span>'
	elif value == "major":
		return '<span style="color:#ffaa7f;">重要</span>'
	elif value == "minor":
		return '<span style="color:#ffff00;">普通</span>'
	elif value == "waring":
		return '<span style="color:#0055ff;">提醒</span>'
