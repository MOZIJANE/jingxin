...
def num2date(value):
	...
	return datetime.datetime.fromtimestamp(dt.timestamp()) - datetime.timedelta(hours=8) #去掉时区信息  
...
def vline(x,color):
	plt.gca().axvline(x=x,color=color)
	
def hline(y,color):
	plt.gca().axhline(y=y,color=color)	
	
def vspan(xmin,xmax,color,alpha=1.0):
	plt.axhspan(xmin,xmax,facecolor=color,alpha=alpha)
	
def hspan(ymin,ymax,color,alpha=1.0):
	plt.axvspan(ymin,ymax,facecolor=color,alpha=alpha)
...
def colors():
	...
	return [x["color"] for x in matplotlib.pylab.rcParams["axes.prop_cycle"]]
...
#选择某个范围 callback(min,max):
#span_stays：布尔值，是否在鼠标释放后仍然保留选区，默认值为False。 
def onselect(ax,callback,color="blue",keepSelect=True,horizontal=True):
	import matplotlib.widgets as mwidgets
	rectprops = dict(facecolor=color, alpha=0.5)
	span = mwidgets.SpanSelector(ax, callback, "horizontal" if horizontal else "vertical",span_stays=keepSelect,rectprops=rectprops)
	ax._current_span = span
	return 
