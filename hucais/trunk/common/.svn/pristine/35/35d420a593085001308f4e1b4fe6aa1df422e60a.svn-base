...
def alert(msg, acceptStr = '确定', parent = None):
	msgBox = QMessageBox(QMessageBox.Warning, "警告", msg, QMessageBox.NoButton, parent)
	msgBox.addButton(acceptStr,QMessageBox.AcceptRole) 
	ret = msgBox.exec_()				
	return True
	
def info(msg, acceptStr = '确定', parent = None):
	msgBox = QMessageBox(QMessageBox.Information, "信息", msg, QMessageBox.NoButton, parent)
	msgBox.addButton(acceptStr,QMessageBox.AcceptRole) 
	ret = msgBox.exec_()				
	return True
...
def confirm(msg, acceptStr = "确定", rejectStr = "取消", parent = None):
	msgBox = QMessageBox(QMessageBox.Information, " ", msg, QMessageBox.NoButton, parent)
	msgBox.addButton(acceptStr,QMessageBox.AcceptRole)
...
def setValue(ctrl,value): 
	...
	if isinstance(ctrl,ui.numberBox.numberBox):
		...
		return
	elif isinstance(ctrl,PyQt5.QtWidgets.QSlider):
		ctrl.setValue(value)
		return
	#elif isinstance(ctrl,QComboBox):
	#	return ctrl.setText(str(value))
	print("qtutility.setValue() unknown type",ctrl)
	assert 0
