...
class image:
	...
	def __init__(self,img,flag=cv2.IMREAD_COLOR):
		if isinstance(img,str):
			...
			if self.img is None:
				raise IOError("image: can't load image file : "+img)
		else:
			self.img = img
	...
	def toRgb(self):
		...
		if self.img.ndim == 3:
			...
			rgb = cv2.cvtColor(self.img, cv2.COLOR_GRAY2BGR)
		return image(rgb)

	def toqpixmap(self, w, h ):
		'''pyqt5 QPixmap class'''
		im = self.clone().toRgb()
		imCv = cv2.resize(im.img, (w,h))
		imQt = QImage(imCv.data, w, h, 3*w, QImage.Format_RGB888)
		return QPixmap.fromImage(imQt)
	...
	#在point上画上立体的x,y,z轴,point是原点位置,axisLen是长度(米)
	#图像的z轴为镜头方向，x向右，y向下，原点为光心
	def drawAxis(self,point,rvecs,tvecs,cameraMatrix,axisLen=1,style="line",showDist=True):
		if style == "line":
		...
		if style == "line":
			...
			if showDist:
				self.text("dist:%0.2fm"%dist,point,color=(0,0,255),align="center")
		elif style == "cube":
	...
	def drawCorners(self, corners,pattenSize=(11,8)):
		if len(self.img.shape) == 2: # if gray image
			self.img = cv2.cvtColor(self.img, cv2.COLOR_GRAY2BGR)
		cv2.drawChessboardCorners(self.img, pattenSize, corners, True)

	def drawRectangle(self, p1, p2, color=[128,128,128]):
		im = self.toRgb()
		return image(cv2.rectangle(im.img, p1, p2, color=color, thickness=3))
	...
	def sendRos(self,topic="image",frameId="image"):
		import ros2.topic
		import sensor_msgs.msg
		d = sensor_msgs.msg.Image()
		d.header = ros2.msg.header(frameId)
		d.height = self.height
		d.width = self.width
		d.is_bigendian = 0
		d.encoding = "bgr8"
		d.step = 3*self.width			# Full row length in bytes
		d.data = list(self.img.flat)	   		 # actual matrix data, size is (step * rows)
		ros2.topic.send(topic,d,"sensor_msgs/Image")
