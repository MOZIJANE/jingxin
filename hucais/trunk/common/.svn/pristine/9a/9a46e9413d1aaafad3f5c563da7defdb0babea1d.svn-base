...
def ping(ip_str,s_ip=None,showLog=False):
	if s_ip is None:
...
#返回时延
def ping2(ip_str,s_ip=None,showLog=False,timeout=5):
	if s_ip is None:
		r = utility.run_cmd("ping -w %s "%timeout + ip_str,showLog=showLog)
	else:
		r = utility.run_cmd("ping -w %s "%timeout + ip_str + " -S " + s_ip,showLog=showLog)
	if len(r[1]):
		return timeout*1000
	if utility.is_win():
		result = re.findall("(时间=(.*)ms)",r[0])
	else:
		result = re.findall("(time=(.*)ms)",r[0])
	times = []
	for d in result:
		times.append(float(d[1]))
	if times:
