...
def set(section,key,value,update=False):
	_get_cfg().set(section,key,value,update=False)
	if not update:
		return
	import configparser
	pp = _get_cfg().filename
	...
	if type != 3:
		for line in alllines:
			if line == "\n":
				continue
			if re.search("^\[(.*)\]$", line):
				currentSection = line.split("]")[0][1:]
				if re.search("(\[%s\])"%section, line):
					if type == 2: #section存在,key不存在
						tempLines.append(line)
						tempLines.append(key + " = " + str(value))
						continue
					currentSection = section
			if re.search(key, line) and currentSection == section and type == 1: #section存在,key存在
				tempLines.append(key + " = " + str(value))
				continue
			tempLines.append(line)
	else: #section不存在的情况
		tempLines = alllines
		tempLines.append("[%s]"%section)
		tempLines.append(key + " = " + str(value))
