...
class client:
	...
	def open(self,url,recCallback = None):
	...
	def _open(self):
		if self.ws:
			self.ws.close()
		self.ws = websocket.WebSocketApp(self.url,on_message = self._onMessage,on_error = self._onError,on_close = self._onClose)
	...
	def _run(self):
		while not self.isExisted:
			self._open()
			self.ws.run_forever(ping_interval=5,ping_timeout=20)
			if self.isExisted:
				break
			time.sleep(5)
