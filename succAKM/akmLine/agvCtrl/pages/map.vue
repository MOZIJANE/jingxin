          ...
          <el-table :data="agvTable" size="mini">
            <el-table-column key="operation" label="操作" width="80">
              <template slot-scope="scope">
                ...
                <icon name="shopping-basket" scale=".7" class="btn" @click.native="setAllEmpty(scope.row.agvId)"></icon>
                <icon name="hand-lizard" scale=".7" class="btn" @click.native="setAllNothing(scope.row.agvId)"></icon>
              </template>
            </el-table-column>
        ...
        <el-card class="scada-task-list-container" v-if="scadaTaskList.length">
          ...
          <el-table :data="scadaTaskList" size="mini">
            <template v-for="{ prop, label, width } in taskColumns">
              ...
              <el-table-column :key="prop" v-else-if="prop !== 'eId'" :prop="prop" :label="label" :width="width"></el-table-column>
            </template>
          </el-table>
        </el-card>
      </el-col>
...
import { getFloors, getMapStatic, getMapHistory, wmqtt, getTaskList, getBasketList, getAgvStatusList, agvGoHome, agvUnlock, resetBasket } from '@/services';
