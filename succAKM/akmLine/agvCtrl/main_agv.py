...
@scadaUtility.get('/api/agv/agvStatus')
def urlStatus2():
	agv = webutility.get_param("agv")
	mapId = webutility.get_param("mapId")
	ret = {}
	aa = agvControl.getAgvList()
	if agv in aa:
		if aa[agv].mapId == mapId:
			if aa[agv].status:
				ret = aa[agv].status
			ret["isActive"] = aa[agv].isActive
			ret['isLocked'] = aa[agv].isLock()
...
def urlStatus():
	...
	ret = [
		...
		{"name": "在线状态", "fieldId": "isActive", "value": agvList.getAgv(agvId).isActive},
		{"name": "断网状态", "fieldId": "isLinked", "value": agvList.getAgv(agvId).isLinked},
