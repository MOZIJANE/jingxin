...
def set_stdout():
	...
	if not g_stdout:
		g_stdout=sys.stdout
		if not g_handle:
			g_handle=open('stdout.log', 'w+')
		sys.stdout = g_handle
set_stdout()	
	
class bar:
	def __init__(self, title ,total):
		utility.logger().info("start "+title)
		set_stdout()
	...
	def failed(self,msg):
		...
		self.out.write(msg)
		self.out.write('\r\n')
		utility.logger().info("finish failed "+self.title + ", " + msg)
		self.out.flush()
	...
	def _show(self): 
		...
		self.out.flush() 
		self.out.write("\033[01;33m"+ self.title.ljust(30)+"\033[0m")
		console = self.width * self.count / self.total
		if console == self.width:
			self.out.write(" "*self.width+"            \033[22;32m[   OK   ]\033[0m")
			self.out.write('\r\n')
			utility.logger().info("finish "+self.title)
		else:
...
def show(msg,color=QColor(staticMgr.accentColor),add_line=True):
	msg = str(msg)
	global g_stdout
	if color  == "red":
...
def test_menu():
	@menu_item("function a","a")
	def funca():
		show("run in funca")

	@menu_item("function b","b")
	def funcb():
		show("run in funcb")

	@menu_item("function c","c")
	def funcb():
		show("run in funcd")
...
if __name__ == '__main__':	
	#test_menu()
	t = input_char("lalala")
	t2 = input_char("lalala",default="Y")
	show("test","red")
	show("test","yellow")
	show("test","green")
	show("test","default")
