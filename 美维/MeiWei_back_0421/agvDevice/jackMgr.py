...
def jackDown(agvId, timeout=90,loc=None):
	agvCtrl.jackDown(agvId,loc=str(loc))
...
def checkInitStatus(agvId,timeout=90):
...
def jackUpOld(agvId ,timeout,loc = None):
	log.info("************run jackUpOld********************")
...
def jackDownOld(agvId ,timeout,loc = None):
	log.info("************run jackDownOld********************")
