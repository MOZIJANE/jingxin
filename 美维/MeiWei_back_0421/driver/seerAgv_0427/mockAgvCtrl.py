...
def goTargetAndDoWork(agvId, loc, operation, recognize=False,use_pgv=False,pgv_adjust_dist=False,pgv_type=1):
	move(agvId,loc)
	
def jackUp(agvId,speed=5000,distance=99999,loc=None):
	return getAgv(agvId).jackUp()

def jackUpOld(agvId,speed=5000,distance=99999,loc=None):
	return getAgv(agvId).jackUpOld()

def jackDown(agvId,speed=5000,distance=99999,loc=None):
	return getAgv(agvId).jackDown()

def jackDownOld(agvId):
	return getAgv(agvId).jackDownOld()
...
def readIOs(agvId):
	return {'DO':[{'id': 0, 'source': 'normal', 'status': True}, {'id': 1, 'source': 'normal', 'status': True}]}
