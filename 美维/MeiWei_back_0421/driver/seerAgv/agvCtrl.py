...
def goTargetAndDoWork(agvId, loc, operation, recognize=False,use_pgv=False, pgv_adjust_dist=False,ignoreAngle=None,pgv_type=1):
	log.info("--------------------跳转到driver下的seerAgv下的agcCtrl.py中的goTargetAndDoWork函数中-----------------10---------")
	...
	if ignoreAngle is not None:
		data["ignoreAngle"] = ignoreAngle
	log.info('--------------------------准备进去driver中的seerAgv.py下的task函数-----------------（11）--------------')
	agvApi.task(agvId, "robot_task_gotarget_req", data)
...
def forkUpDownStatus(agvId,loc):
	agvApi.conf(agvId,"robot_status_fork_req",None)
...
def jackDownStatus(agvId):
	if checkCfgIO(agvId):
		...
		ret = agvApi.status(agvId, "robot_status_jack_req",None)  #import driver.seerAgv.agvApi as agvApi
		if ret["jack_state"]==3:
			return {"status":1}
	elif isForkType(agvId):
