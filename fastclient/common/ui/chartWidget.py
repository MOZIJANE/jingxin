...
class chartWidget(PyQt5.QtWidgets.QWidget): 
	def __init__(self,figName="",addToolbar=True): 
		...
		if figName == "":
			figName = "fig"+str(id(self))
		self._fig = plt.figure(figName)
		...
		self.homeAct = PyQt5.QtWidgets.QAction(qta.icon('fa.home',color='black'), "&Home", triggered=_wrap(self.home,self))
		self.preAct = PyQt5.QtWidgets.QAction(qta.icon('fa.undo',color='black'), "&Pre view", triggered=_wrap(self.back,self))
		self.forwardAct = PyQt5.QtWidgets.QAction(qta.icon('fa.repeat',color='black'), "&Next view", triggered=_wrap(self.forward,self))
		self.zoomAct = PyQt5.QtWidgets.QAction(qta.icon('fa.search',color='black'), "&Zoom", triggered=_wrap(self.zoom,self))
		self.zoomAct.setCheckable(True) 
		self.moveAct = PyQt5.QtWidgets.QAction(qta.icon('fa.arrows',color='black'), "&Move", triggered=_wrap(self.move,self))
		...
		self.saveAct = PyQt5.QtWidgets.QAction(qta.icon('fa.save',color='black'), "&Save", triggered=_wrap(self.save,self)) 
	...
	def _updateToolbar(self):   
		if self._plotToolbar._active == 'PAN':
			self.moveAct.setChecked(True)
		else:
			self.moveAct.setChecked(False)
		if self._plotToolbar._active == 'ZOOM':
