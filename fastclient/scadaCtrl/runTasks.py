...
def run():
	...
	while g_taskMgr.runningCount(isInside=True) < MAX_INSIDE_AGV_NUM and len(g_taskInsideQueue) > 0:
			...
			for t in g_taskInsideQueue:
				...
				break
			if findTask:
				g_taskInsideQueue.remove(findTask)
...
class scadaTask:
	...
	def onFinish(self,obj):
		...
		if not obj["result"]:
			...
			if self.taskStep == "loadPlayload":
				# taskMgr.error(self.taskId, failMsg="二维读取或识别货架腿失败，" + obj["resultDesc"])
				self.taskStep = "failed"
				taskMgr.fail(self.taskId, failMsg="二维读取或识别货架腿失败，%s"%obj["resultDesc"])
			else:
		...
		if self.taskStep == "moveSource" or self.taskStep == "startMovePreSrc": 
			...
			taskMgr.update(self.taskId,failMsg="识别二维码，AGV顶升")
			return
...
#			lockImp.release(g_lock)
			...
			taskMgr.update(self.taskId,failMsg="等待目标位置%s前置点锁"%self.target)
			return True
			...
			taskMgr.update(self.taskId,failMsg="等待前置点%s与工位点%s处无小车" % (self.t_info["prepoint"], self.t_info["location"]))
			return True
	...
	def _filterFunc(self, agvId, mapId, loc, payload):
		...
		return True
	# 过滤没有初始化成功的车
	def _filterFunB(self, agvId, mapId, loc, payload):
		status = jackApi.checkInitStatus(agvId, timeoutSec=90)["status"]
		if status == 1:
			return True
		log.warning(" %s :DO37 is 0,canot get task." %agvId)
		return False

	#@lockImp.lock(g_lock)
	def _applyAgv(self): 
		try:
			return agvApi.apply(taskId=self.param["taskId"], mapId=self.param["floorId"],loc=self.param["location"], filterFunc=self._filterFunB, timeout=10) #filterFunc=self._filterFunc,
		except Exception as e:
	#		lockImp.release(g_lock)
	...
	def _moveAgvPrepoint(self,stepCallback=None):
		try:
			self.param["timeoutSec"] = 86400
	...
	def _moveJackup(self):
		self.param["timeoutSec"] = 86400
		zloc = ["AP750","AP753","AP748","AP749"]
		if "location" in self.param and self.param["location"] in zloc:
			agvApi.moveJackup(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, recognize=True,use_pgv=True,pgv_type=4)
		elif 'WorkPlace' in self.param and self.param['WorkPlace'] and self.param['WorkPlace']!='' and self.param['WorkPlace'].lower() == 'true':
			agvApi.moveJackup(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, use_pgv=False)
		else:
			if self.isInside:
				agvApi.moveJackup(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, recognize=True,use_pgv=True,pgv_type=4)
			else:
				agvApi.moveJackup(self.agvId, self.param["floorId"]+'.'+self.param["location"], self.onFinish, self.param, recognize=False,use_pgv=True,pgv_type=4)
	#@lockImp.lock(g_lock)
