...
class agvInfo:
	...
	def moveJackup(self, loc, timeout, taskId, use_pgv=False,ignoreAngle=False,pgv_type=1,recognize=False):
		...
		if self.isSeerRobot():
			agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="ForkLoad",recognize=recognize,use_pgv=use_pgv,pgv_type=pgv_type)
		else:
			agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="ForkLoad",recognize=recognize,use_pgv=use_pgv,ignoreAngle=ignoreAngle,pgv_type=pgv_type)
	...
	def moveJackdown(self, loc, timeout, taskId,ignoreAngle=False):
		...
		if self.isSeerRobot():
			...
			agvCtrl.goTargetAndDoWork(self.agvId, loc, operation="ForkUnload",recognize=False,use_pgv=False,ignoreAngle=ignoreAngle)
			...
			for a in alarms:
				for k in a:
					...
					if id in clearTypeIds:
						...
						if id == 54901:
							log.info("clearTypeIds: %s and _allAlarmId:" % clearTypeIds,self._allAlarmId)
