...
def urlMove():
	...
	use_pgv = webutility.get_param("use_pgv")
	pgv_type = webutility.get_param("pgv_type")
	if isinstance(use_pgv, str):
		if use_pgv=='False':
			use_pgv=False
		else:
			use_pgv=True
...
def urlStatusList():
	log.info('请求了agvCtrl下的agvStatusList')
	mapId = webutility.get_param("mapId")
	ret = {}
		...
		for a in aa:
			if aa[a].mapId == mapId:
				if aa[a].status:
					...
					ret[a]["isBlocked"] = aa[a].isDeadLock()
				else:
					ret[a]=dict()
...
@scadaUtility.get('/api/agv/checkLineIdle')
def urlLineIdle():
	agvId = webutility.get_param("agv")
	start = webutility.get_param("start")
	end = webutility.get_param("end")
	map = agvList.getAgv(agvId).map
	if map is None:
		return {"result": True}
	line = map.getLine(start,end)
	if line is None:
		return {"result": True}
	agvs = line.agvs
	for a in agvs:
...
def urlStatus():
	...
	task_type = ["没有任务", "自由导航到任意点", "自由导航到站点", "固定路径导航到站点", "其他","","","平动转动"]
	...
	ret = [
		...
		{"name": "堵塞状态","fieldId": "isBlocked","value": agvList.getAgv(agvId).isDeadLock()},
		...
		{"name": "任务状态","fieldId": "task_status","value": "其他" if str(info["task_status"])=="100" else task_status[info["task_status"]]}, 
...
if __name__ == '__main__': 		 
	if webutility.is_bottle():
		...
		import threading
		import mainFrame 
		if local.getbool("project","showUI",True):
