...
NEAR_POS_DISTANCE = local.getfloat("project","nearPosDistance",0.2)
...
class mapInfo:
	...
	def getAgvPath(self,agvId):
		...
		for r in ret1:
			...
			#添加block.cfg路径 
			lines = self.blockCfg.getGroup(r)
			find = False
			...
			for line in lines:
				if str(line) not in ret2:
					ret2.append(str(line))
	...
	def getPosByPoint(self, x, y):
		...
		for p in self.positions:
			if p.contains(x, y):
				d = p.distance(x,y)
				if p.isApType and d >= NEAR_POS_DISTANCE*1.5:
					continue
				elif d >= NEAR_POS_DISTANCE: #相差0.2米，才认为到达 
					continue
				pp.append((p,d))
		if not pp:
			return None
	...
	def block(self, agvId, startPosName, endPosName, paths):   
		...
		if len(checkLines) == 0: 
			log.error(agvId+" block failed, mapId:"+str(self.mapId)+",unknow line:" + startPosName + ", paths:" + ",".join([ str(x) for x in paths ])) 
			return False
