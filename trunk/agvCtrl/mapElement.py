...
class agvPoint:
	...
	def blockPath(self):   
		...
		y = self.y
		r = self.r
		key = strPaths(lines) + "%0.3f_%0.3f_%0.3f_%0.3f_%0.3f"%(w,h,x,y,r)
		#if key == self.blockPathKey:
		#	return self.blockPathCache
		
		for i in range(len(lines)):
			#print("add line",str(lines[i]))
			line = lines[i]  
			if box is None:
				box = line.blockPath(x=x,y=y,w=w,h=h)
				box.setFillRule(1)
				#开始点会带旋转
				if line.startPos.contains(x,y):
					p = bezierPath.rotate(x,y,r,line.r,w,h)
					box.addPath(p)
			else:	
		...
		if box is None:
			return None
		#self.blockPathKey = key
		#self.blockPathCache = box
	...
	def tryCheck(self, agvs, blockPath=None):
	...
	# 如果被阻挡，则返回AGVID 
	#判断将要走的路径，是否有交集
	def check(self, agvs, blockPath=None):
		otherId = self.tryCheck(agvs,blockPath=blockPath)
		if otherId:
			if not self.checkFailed:
...
class advancedCurve:
	...
	def boundPath(self,w,h):
		key = "%0.3f_%0.3f"%(w,h)
		if key in self._lastBound:
			return self._lastBound[key]
		p = bezierPath.strock(self._ptrList,width=w,height=h)
		self._lastBound[key] = p
		return p
