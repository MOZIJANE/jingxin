...
def getUnfinishTask():
	...
	return db.find("u_smt_task", {"status": {"$nin": ["finish","fail"]},"createTime": {"$gte": t}}).list()
	
def updateFailTask():
	t = datetime.datetime.now()
	t = t - datetime.timedelta(days=8)
	ds = db.find("u_smt_task", {"status": {"$in": ["waiting","running","error"]},"createTime": {"$gte": t}})
...
def start(taskId,**data):
	...
	data["updateTime"] = now
	data["status"] = "running"
	db.update_one("u_smt_task",{"_id": db.ObjectId(taskId)},data)
...
def get(taskId):
	ret = db.find_one("u_smt_task",{"_id": db.ObjectId(taskId)})
	if ret is None:
		return None
	_update(ret)
	return ret
...
def getTaskList(taskList):
	if taskList is None or not taskList:
		...
		ds = db.find("u_smt_task",{"_id": {"$in": t}})
	s = []
	while ds.next():
		t = {
			"taskId": str(ds["_id"]),
...
def getTaskList2():
	...
	ds = db.find("u_smt_task", {"createTime": {"$gte": t}}).sort('createTime', db.DESCENDING)
	s = []
...
######### unit test #########
def test_add():
	db.delete_many("u_smt_task",{"type":"testhhh"})
...
if __name__ == '__main__':
	...
	print (db.find("u_smt_task", {"status": {"$nin": ["finish","fail"]},"createTime": {"$gte": t}}).list())
